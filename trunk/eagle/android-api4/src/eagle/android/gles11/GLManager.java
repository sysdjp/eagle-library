...
*/
	...
	public	void			fillRect( int x, int y, int w, int h, float rotateDeg, int colorRGBA )
	{
		...
		//!	描画色指定
		gl.glColor4x(	( ( colorRGBA >> 24 ) & 0xff ) * 0x10000 / 255,
						( ( colorRGBA >> 16 ) & 0xff ) * 0x10000 / 255,
						( ( colorRGBA >> 8 )  & 0xff ) * 0x10000 / 255,
						( ( colorRGBA & 0xff ) ) * 0x10000 / 255
						);

		//!	描画位置を行列で操作する
		{
			float	sizeX = ( float )w / ( float )getDisplayWidth() * 2,
					sizeY = ( float )h / ( float )getDisplayHeight() * 2,
					sx = ( float )x / ( float )getDisplayWidth() * 2,
					sy = ( float )y / ( float )getDisplayHeight() * 2;
	...
	public	void			drawImage(	int x, int y, int w, int h,
										float rotateDeg, int colorRGBA,
										ITexture tex, int tx, int ty, int tw, int th )
	{
		GL11	gl = gl11;
		...
		//!	UV操作
		{
			...
			float	sizeX = ( float )tw / ( float )tex.getWidth(),
					sizeY = ( float )th / ( float )tex.getHeight(),
					sx = ( float )tx / ( float )tex.getWidth(),
					sy = ( float )tx / ( float )tex.getHeight();
	...
	{
		result.set(		( screen.x * getDisplayWidth( ) + getDisplayWidth( ) ) / 2
					,	getDisplayHeight( ) - ( screen.y * getDisplayHeight( ) + getDisplayHeight( ) ) / 2
					,	screen.z
						);
